...
export async function sendSMSOTP(phoneNumber: string, otp: string): Promise<{ success: boolean; message?: string }> {
  try {
    ...
    const senderId = process.env.MNOTIFY_SENDER_ID || 'COMPSSA'
    ...
    const message = `Your COMPSSA voting verification code is: ${otp}. This code expires in 10 minutes. Do not share this code with anyone.`
